  ...
  // Otherwise, normalize & parse the connection URL.
    ...
    if (expectedProtocolPrefix) {
      if (dsConfig.url.match(/^:\/\//)) {
        dsConfig.url = dsConfig.url.replace(/^:\/\//, expectedProtocolPrefix+'://');
      }
      else if (!dsConfig.url.match(/^[a-zA-Z][a-zA-Z0-9+.-]*:\/\//)) {
        dsConfig.url = 'mongodb://' + dsConfig.url;
      }
    }//>-


    // Now attempt to parse out the URL's pieces and validate each one.
    var parsedConnectionStr = url.parse(dsConfig.url);
    ...
    // > when we rebuild it.  But we do fold them into the dsConfig dictionary at
    // > the top level as well -- unless there is a conflict.
