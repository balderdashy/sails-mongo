...
module.exports = function doWithConnection(options, done){
  ...
  (function _ensureConnection(proceed){
    ...
    if (options.driver.getConnection.sync) {
      var connection;
      try {
        connection = options.driver.getConnection({ manager: options.manager }).execSync().connection;
    ...
    (function _makeCallToDuringFn(proceed){
      ...
      if (options.driver.releaseConnection.sync) {
        try {
          options.driver.releaseConnection({ connection: db, meta: options.meta }).execSync();
          // (`report.meta` is ignored...)
        } catch (secondaryErr) {
          return done(new Error(
