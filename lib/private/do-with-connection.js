  ...
  (function _ensureConnection(proceed){
    ...
    (function _makeCallToDuringFn(proceed){
      ...
      if (duringErr) {
        ...
        options.WET_MACHINES.releaseConnection({ connection: db, meta: options.meta, manager: options.manager }).switch({
      ...
      if (options.WET_MACHINES.releaseConnection.sync) {
        try {
          options.WET_MACHINES.releaseConnection({ connection: db, meta: options.meta, manager: options.manager }).execSync();
          // (`report.meta` is ignored...)
        } catch (secondaryErr) {
          return done(new Error(
