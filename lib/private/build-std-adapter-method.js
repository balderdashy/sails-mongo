    ...
    doWithConnection({
      ...
      during: function (connection, proceed) {
        ...
        performQuery({
          ...
          phOrm: { models: registeredPhModels }
        }).exec({
          error: function (err) { return proceed(err); },
          notUnique: function (err) { return proceed(err); },//<< `footprint` is already attached
          success: function (result) {

            // Skip transformation if no handler was provided.
            if (!handleTransformingResultSync) {
              return proceed(undefined, result);
            }

            // But otherwise, run the transformation handler before
            // sending back the transformed result.
            var transformedResult = handleTransformingResultSync(result);
