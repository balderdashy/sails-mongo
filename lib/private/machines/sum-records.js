module.exports = {
  ...
  fn: function sum(inputs, exits) {
    ...
    let numericFieldName = s3q.numericAttrName;

    // Grab the model definition
    let WLModel = _.find(inputs.dryOrm.models, {tableName: tableName});
    if (!WLModel) {
    ...
    cursor.toArray().then(function aggregateCb(nativeResult) {
      let sum = 0;
