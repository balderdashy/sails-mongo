module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    var database = _clientConfig.database;
    _clientConfig = _.omit(_clientConfig, ['url', 'user', 'password', 'host', 'port', 'database']);

    // so that it uses the new URL string parser. Old one is deprecated
    _.merge(_clientConfig, {useNewUrlParser: true});
    ...
    NodeMongoDBNativeLib.MongoClient.connect(mongoUrl, _clientConfig, function connectCb(err, client) {
      ...
      // `client` will be our manager. The callback used to return `db`, but that changed in latest mongodb
      // driver versions. Now it returs the MongoClient instead of the database
      ...
      manager._WPdb = manager.db(database);
