module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    var mongoWhere;
    try {
      mongoWhere = reifyWhereClause(s3q.criteria.where, WLModel, s3q.meta);
    } catch (e) { return exits.error(e); }
    ...
    (function findMatchingIdsMaybe(proceed) {
      ...
      if (!isFetchEnabled) {
        ...
        // console.log('%d matching',pkValsOfMatchingRecords.length);
        // console.log('secondaryMongoWhere',require('util').inspect(secondaryMongoWhere,{depth:5}),'');
        // console.log('typeof the first id',typeof pkValsOfMatchingRecords[0]);
        // console.log('typeof the first id after putting it into secondaryMongoWhere',typeof secondaryMongoWhere[pkColumnName]['$in'][0]);
      }

      // console.log('values to set:',s3q.valuesToSet);
      mongoCollection.updateMany(secondaryMongoWhere, { '$set': s3q.valuesToSet }, function updateManyCb(err) {
        ...
        // But otherwise we'll re-fetch the now-updated records.

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // TODO: Handle case where pk value was changed!
        // (we'll need to swap out the id)
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

        // TODO: get rid of this debug stuff
        // mongoCollection.find().toArray(function (err, phRecords) {
        mongoCollection.find(secondaryMongoWhere).toArray(function (err, phRecords) {
          if (err) { return exits.error(err); }

          // console.log('%d found in 3rd query',phRecords.length);
          // console.log('3rd query results',require('util').inspect(phRecords,{depth:5}));
          // console.log('typeof _id in first record returned from 3rd query',typeof phRecords[0]._id);
