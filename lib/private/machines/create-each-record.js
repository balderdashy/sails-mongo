module.exports = {
  ...
  fn: function (inputs, exits) {
    ...
    // }
    (function(iifeDone){  mongoCollection.insertMany(s3q.newRecords).then(function(nativeResult) { iifeDone(null, nativeResult);}).catch(function(err) { iifeDone(err, null);});})(function(err, nativeResult) {
      if (err) {
      ...
      (function(iifeDone){ mongoCollection.find({ _id: { $in: insertedIds } }).toArray().then(function(phRecords) { iifeDone(null, phRecords); }).catch(function(err) { iifeDone(err, null);});})(function(err, phRecords) {
        if (err) {
          return exits.error(err);
        }
        try {
