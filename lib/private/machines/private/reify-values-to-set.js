...
module.exports = function reifyValuesToSet(valuesToSet, WLModel) {
  ...
  if (_.has(valuesToSet, primaryKeyColumnName)) {
    console.log('Explicit value (`%s`) was set for primary key (%s)',valuesToSet[primaryKeyColumnName], primaryKeyColumnName);
    try {
      valuesToSet[primaryKeyColumnName] = normalizeMongoObjectId(valuesToSet[primaryKeyColumnName]);
    } catch (e) {
      switch (e.code) {
