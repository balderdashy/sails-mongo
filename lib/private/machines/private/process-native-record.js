...
function isObjectIdOrHexString(v) {
  return isBsonType(v, 'ObjectId') ||
  (typeof v === 'string' && objectIdHexRegexp.test(v) || ObjectId.isValid(v));
}
...
module.exports = function processNativeRecord(nativeRecord, WLModel, meta) {
  ...
  if (useObjectIds) {
    ...
    if (_.isObject(pkValue) && isObjectIdOrHexString(pkValue)) {
  ...
  _.each(WLModel.attributes, function (attrDef) {
    ...
    if (!meta || !meta.modelsNotUsingObjectIds || !_.contains(meta.modelsNotUsingObjectIds, attrDef.model)) {
      if (_.isObject(nativeRecord[phRecordKey]) && isObjectIdOrHexString(nativeRecord[phRecordKey])) {
