...
module.exports = function processNativeRecord(nativeRecord, WLModel, meta) {
  ...
  // While we're at it, we'll also detect any `type: 'ref'` attributes that were
  // retrieved as Binary objects, and transform them back to Buffer objects.
  ...
  _.each(WLModel.attributes, function (attrDef) {
    ...
    if (attrDef.type === 'ref' && nativeRecord[phRecordKey] instanceof Binary && nativeRecord[phRecordKey].buffer) {
