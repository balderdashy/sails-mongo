module.exports = require('machine').build({
  ...
  inputs: {
    ...
    manager: {
      description: 'The manager to lease a connection from (unless `connection` is supplied).',
      required: true,
      readOnly: true,
      example: '===',
    },
    ...
    connection: {
      description: 'An existing connection to use (otherwise, by default, a new connection is acquired from the manager).',
      extendedDescription: 'If unspecified, the manager will be used instead.',
      readOnly: true,
      example: '===',
    },
  ...
  fn: function (inputs, exits) {
    ...
    if (!_.isUndefined(inputs.connection)) {
      db = inputs.connection;
    }
    else {
      // NOTE: This is sails-mongo, so if no explicit connection was specified,
      // we can just use the manager as the connection.  (Otherwise, with other
      // databases, we might need to get and release the connection.)
      db = inputs.manager;
    }
