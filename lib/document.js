...
Document.prototype.serializeValues = function serializeValues(values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    // Turn Foreign Keys into ObjectIds
    if(hasOwnProperty(self.schema[key], 'foreignKey') && _.isString(values[key]) && values[key].match(/^[a-fA-F0-9]{24}$/)) {
      /**
       * @todo Replace try/catch with check for foreign keys that are not ObjectIDs (i.e. where joined table has autoPK:false)
       */
      try {
        values[key] = new ObjectId.createFromHexString(values[key].toString());
      } catch (e) {
      }
