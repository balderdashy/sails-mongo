...
module.exports = {
   ...
   *               ˙ definition: {Dictionary}  :: The physical-layer report from waterline-schema.  NOTE THAT THIS IS NOT A NORMAL MODEL DEF! It's more similar to `attributes`, but not even that, really.
   *                             ˚¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯¯\
  ...
  registerDatastore: function (dsConfig, physicalModelsReport, done) {
    ...
    //  ╝╚╝╚═╝╩╚═╩ ╩╩ ╩╩═╝╩╚═╝╚═╝  ─┴┘┴ ┴ ┴ ┴ ┴└─┘ ┴ └─┘┴└─└─┘  └─┘└─┘┘└┘└  ┴└─┘
    try {
      normalizeDatastoreConfig(dsConfig);
    } catch (e) {
      switch (e.code) {
    ...
    // ============================================================================================
    if (module.exports.verifyModelDef) {

      var modelIncompatibilitiesMap = {};
      try {
        var verifyModelDef = Machine.build(module.exports.verifyModelDef);
        _.each(physicalModelsReport, function (phModelInfo){
          try {
            verifyModelDef({ modelDef: phModelInfo }).execSync();
          } catch (e) {
            switch (e.exit) {
              case 'malformed': modelIncompatibilitiesMap[phModelInfo.identity] = e.output; break;
              default: throw e;
            }
          }
        });//</_.each()>
      ...
      success: function (report) {
        try {
          ...
          _.each(physicalModelsReport, function(phModelInfo){
            ...
            registeredPhModels[phModelInfo.identity] = {
            ...
            //       { type: 'string',
            //         columnName: '_id',
            //         autoMigrations: { columnType: '_integer_etc', autoIncrement: true, unique: true } },
            //      firstName:
            //       { type: 'string',
            //         autoMigrations: { columnType: '_varchar_etc' },
            ...
            //      age:
            //       { type: 'number',
            //         autoMigrations: { columnType: '_integer_etc' },
            //         validations: { min: 0, max: 140 },
            //         columnName: 'age' },
            //      obj:
            //       { type: 'json',
            //         autoMigrations: { columnType: '_json_etc' },
            //         columnName: 'obj' },
            //      status:
            //       { type: 'boolean',
            //         autoMigrations: { columnType: '_boolean_etc' },
            //         columnName: 'status' },
            //      avatar:
            //       { type: 'ref',
            //         autoMigrations: { columnType: '_text_etc' },
            //         columnName: 'avatar' },
            //      sort:
            //       { type: 'json',
            //         autoMigrations: { columnType: '_json_etc' },
            ...
            //       { type: 'number',
            //         autoUpdatedAt: true,
            //         autoMigrations: { columnType: '_bigint_etc' },
  ...
  teardown: function (datastoreName, done) {
    ...
    //  ═╩╝╚═╝╚═╝ ╩ ╩╚═╚═╝ ╩   ┴ ┴┴ ┴┘└┘┴ ┴└─┘└─┘┴└─
    // Destroy the manager.
    WLDriver.destroyManager({ manager: dsEntry.manager }, {
      ...
      success: function (report) {
        ...
        try {
          ...
          _.each(_.keys(registeredPhModels), function(modelIdentity) {
            if (registeredPhModels[modelIdentity].datastore === datastoreName) {
              delete registeredPhModels[modelIdentity];
            }
          });
  ...
  verifyModelDef: {
    ...
    exits: {
      invalid: { outputExample: 'Primary key attribute should have `columnName: \'_id\'`.' }
    },
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  create: buildStdAdapterMethod(require('./private/machines/create-record'), WLDriver, registeredDsEntries, registeredPhModels, function __handleTransformingResultSync__(report) {
    ...
    // > for consistency.
    if (_.isNull(report.record)) { return undefined; }
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  createEach: buildStdAdapterMethod(require('./private/machines/create-each-record'), WLDriver, registeredDsEntries, registeredPhModels, function __handleTransformingResultSync__(report) {
    // (`report.meta` is ignored)

    // > Note: we use this `if` statement because, if `report.records` was undefined,
    // > it will have been coerced to null-- but we want it to be `undefined` still,
    // > for consistency.
    if (_.isNull(report.records)) { return undefined; }
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  update: buildStdAdapterMethod(require('./private/machines/update-records'), WLDriver, registeredDsEntries, registeredPhModels, function __handleTransformingResultSync__(report) { /*This is identical to createEach's and so was condensed for brevity -- see above for details.*/ if (_.isNull(report.records)) { return undefined; } return report.records; }),
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  destroy: buildStdAdapterMethod(require('./private/machines/destroy-records'), WLDriver, registeredDsEntries, registeredPhModels, function __handleTransformingResultSync__(report) { /*This is identical to createEach's and so was condensed for brevity -- see above for details.*/ if (_.isNull(report.records)) { return undefined; } return report.records; }),
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  find: buildStdAdapterMethod(require('./private/machines/find-records'), WLDriver, registeredDsEntries, registeredPhModels),
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  count: buildStdAdapterMethod(require('./private/machines/count-records'), WLDriver, registeredDsEntries, registeredPhModels),
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  sum: buildStdAdapterMethod(require('./private/machines/sum-records'), WLDriver, registeredDsEntries, registeredPhModels),
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  avg: buildStdAdapterMethod(require('./private/machines/avg-records'), WLDriver, registeredDsEntries, registeredPhModels),
