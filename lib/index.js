...
module.exports = {
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  create: function (datastoreName, query, done, meta) {
    ...
    // Determine the appropriate connection to use, leasing one if necessary.
    (function __beforeProceeding__(proceed) {

      if (meta && meta.leasedConnection) {
        return proceed(undefined, meta.leasedConnection);
      }//-•

      if (WLDriver.getConnection.sync) {
        var connection;
        try {
          connection = WLDriver.getConnection({ manager: dsEntry.manager }).execSync().connection;
          // (`report.meta` is ignored)
        } catch (e) { return proceed(e); }
        return proceed(undefined, connection);
      }//-•

      WLDriver.getConnection({ manager: dsEntry.manager }, {
      ...
      helpCreate({
        ...
        error: function (err) { return done(err); },
        notUnique: function (err) { return done(err); },//<< `footprint` is already attached
        success: function (report) {
          // (`report.meta` is ignored)
