...
module.exports = {
  ...
  // to the static Mongo library for Node.js. (See https://npmjs.com/package/mongodb)
  mongodb,
        ...
        try {
          ...
          _.each(physicalModelsReport, (phModelInfo) => {
            ...
            if (registeredDryModels[phModelInfo.identity]) {
              throw new Error(`Consistency violation: Cannot register model: \`${phModelInfo.identity}\`, because it is already registered with this adapter!  This could be due to an unexpected race condition in userland code (e.g. attempting to initialize multiple ORM instances at the same time), or it could be due to a bug in this adapter.  (If you get stumped, reach out at https://sailsjs.com/support.)`);
            }
  ...
  teardown(datastoreName, done) {
    ...
    // Sanity checks:
    if (!datastoreName) {
      return done(new Error(`Consistency violation: Internal error in Waterline: Adapter received unexpected falsey datastore name (\`${datastoreName}\`)!  Can't look up a DS entry from this adapter with that...  (Please report this error at https://sailsjs.com/bugs.)`));
    }
  ...
  define(datastoreName, tableName, phmDef, done) {
    ...
    async.each(uniqueIndexesToCreate, (key, next) => {
      ...
      mongoCollection.createIndex(mongoSingleFieldIdxKeys, { unique: true }, (err) => {
