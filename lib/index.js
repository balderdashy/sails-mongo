...
var helpCreate = require('./private/machines/create');
...
module.exports = {
  ...
  registerDatastore: function (dsConfig, physicalModelsReport, done) {
    ...
    // Validate datastore configuration (for this adapter-specifically):
    // ============================================================================================
    try {
      module.exports.validateDatastoreConfigStrict({ dsConfig: dsConfig }).execSync();
    } catch (e) {
      switch (e.exit) {
        case 'malformed': return done(flaverr('E_DATASTORE_NOT_COMPATIBLE', new Error('Invalid configuration for datastore `' + datastoreName + '`:  '+e.output)));
        default: return done(e);
      }
    }


    // Validate models (for this adapter-specifically):
    // ============================================================================================
    var modelIncompatibilitiesMap = {};
    try {
      _.each(physicalModelsReport, function (phModelInfo){
        try {
          module.exports.verifyModelDef({ modelDef: phModelInfo }).execSync();
        } catch (e) {
          switch (e.exit) {
    ...
    WLDriver.createManager({
      ...
      malformed: function(report) {
        var err = new Error('The given connection URL is not valid for this database.  Details:\n```\n'+report.error.stack+'\n```');
        err.raw = report.error;
        if (report.meta) { err.meta = report.meta; }
        return done(err);
      },
      failed: function(report) {
        var err = new Error('Failed to connect with the given datastore configuration.  Details:\n```\n'+report.error.stack+'\n```');
        err.raw = report.error;
        if (report.meta) { err.meta = report.meta; }
        return done(err);
      },
  ...
  //  ╚╗╔╝║╣ ╠╦╝║╠╣ ╚╦╝  ║║║║ ║ ║║║╣ ║     ║║║╣ ╠╣
  //   ╚╝ ╚═╝╩╚═╩╚   ╩   ╩ ╩╚═╝═╩╝╚═╝╩═╝  ═╩╝╚═╝╚
  verifyModelDef: require('machine').build({
    ...
    sync: true,
    inputs: {
      modelDef: { example: {}, required: true }
    },
  ...
  validateDatastoreConfigStrict: require('machine').build({
    description: 'Ensure that the specified datastore config is compatible with this adapter.',
    extendedDescription: 'This assumes that the provided config has already undergone adapter-agnostic normalization, and is considered generally valid.',
    sync: true,
    inputs: {
      dsConfig: {example: {}}
    },
   ...
   * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  create: function (datastoreName, s3q, done) {
      ...
      during: function (connection, proceed) {
        ...
        helpCreate({
          ...
          success: function (report) {
            ...
            // > for consistency.
            if (!report.record) { return proceed(); }
            return proceed(undefined, report.record);
          }
        });
