...
var Query = module.exports = function Query(options) {
  ...
  // Hold any "joins" used to build up nested documents
  this.joins = [];

  // Check for any Waterline "joins"
  this.buildJoins(options);
...
/**
 * Build any Waterline "joins"
 *
 * Grabs the 'joins' key if it exist in the options for doing in-memory joins
 * later on the related documents.
 *
 * @param {Object} options
 * @api private
 */

Query.prototype.buildJoins = function buildJoins(options) {
  if(!hasOwnProperty(options, 'joins')) return options;

  this.joins = _.cloneDeep(options.joins);
  ...
  if(hasOwnProperty(options.where, '_id')) {
    ...
    if(Array.isArray(options.where['_id'])) {
      options.where['_id'] = options.where['_id'].map(function(id) {
        return utils.matchMongoId(id) ? new ObjectId(id) : id;
      });
    }
...
Query.prototype.parseOptions = function parseOptions(key, obj, original, parent) {
  ...
  Object.keys(obj).forEach(function(attr) {
    ...
    if(utils.matchMongoId(obj[attr])) {
      original[attr] = ObjectId(obj[attr].toString());
      return;
    }
