...
var utils = module.exports = {
  ...
  //  ╝╚╝╚═╝╩╚═╩ ╩╩ ╩╩═╝╩╚═╝╚═╝  ╚═╝╩╚═╩ ╩ ╚═╝╩╚═╩╩ ╩
  //
  // Converts any id properties into _id for a valid mongo criteria.
  normalizeCriteria: function normalizeCriteria(criteria) {
    // Build a parse function that can be called recursively
    var parse = function parse(data) {
      _.each(data, function inspectKeyVal(val, key) {
        if (key === 'id') {
          data._id = val;
          delete data.id;
        }
