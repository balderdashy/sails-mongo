...
// Private var to track of all the datastores that use this adapter.  In order for this adapter
// to be able to connect to the database, we expose this var publicly as well.
...
// > the Waterline adapter spec (a breaking change).
...
module.exports = {
  ...
  teardown: function teardown(identity, cb) {
    ...
    if (!identity) {
      datastoreIdentities = datastoreIdentities.concat(_.keys(registeredDatastores));
    } else {
