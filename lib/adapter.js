          ...
          connection.collection(collectionName, function(err, collection) {
            ...
            function processKey(key, cb) {
              ...
              if( def[key].unique || def[key].index ) {
                ...
                index = {};
                //allow for custom defined indexes
                if( typeof def[key].index === "string"){
                  index[key] = def[key].index;
                //if not a custom index, use a regular index
                }else{
                  index[key] = 1;
                }
              
                //set up the options for the index
                options = {
                  sparse:true
                };
                ...
                return collection.ensureIndex(index, options, function(err) {
                  if(err) return cb(err);
