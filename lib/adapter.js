...
module.exports = (function() {
  ...
  var adapter = {
    ...
    registerCollection: function(collection, cb) {
      ...
      dbs[collection.identity] = _.find(dbs, function(db) {
        return _(collection.config).isEqual(db.config);
      });
    ...
    native: function(collectionName, cb) {
      ...
      var db = dbs[collectionName];
      var connection = db.connection;
      if(connection) {
        return afterwards();
      }

      createConnection(db.config, function(err, newConnection) {
  ...
  function spawnConnection(logic, config, cb) {
    ...
    // Grab the existing connection from dbs
    var db = _.find(dbs, function(db) {
      return _(config).isEqual(db.config);
    });
    ...
    createConnection(config, function(err, newConnection) {
      db.connection = newConnection;
      connection = newConnection;
      afterwards();
    });
