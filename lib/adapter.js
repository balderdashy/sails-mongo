...
module.exports = (function() {
  ...
  var adapter = {
    ...
    teardown: function (conn, cb) {
      ...
      if (conn == null) {
        teardowns = Object.keys(connections);
      } else {
        teardowns.push(conn);
      }

      async.each(teardowns, function(name, next) {
    	if(!connections[name]) return next();

        var dbConnection = connections[name].connection.db;
